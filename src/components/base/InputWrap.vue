...
<template>
  <div class="input-wrap" :class="{
    ...
    <template v-if="ticker">
      ...
      <div v-if="currency" class="input-ticker-currency" :style="`height: ${inputHeight}`">
        {{ currency }}
      </div>
    </template>

    <div v-if="dbEdit && !flags.edit" class="input-wrap-data" @dblclick="flags.edit = 1">
      <span>
        {{ value }}
      </span>
    ...
    <input ref="input" v-if="flags.edit || !dbEdit" v-focus:dbEdit="dbEdit" :type="type" :value="newValue"
      :readonly="readOnly" v-bind="$attrs" v-on="$listeners" @input="handleInput" @keydown.esc="handleEscape"
      @keydown.enter="handleSave" @keypress="validate($event, type)" />

    <span v-if="extra" class="input--extra">
      {{ extra }}
    </span>

    <i v-if="icon" :class="`ico-${icon}`">
    </i>

    <div v-if="arrows" class="input-arrows">
      <div class="ico-triangle-up" @click="action('increment')">
      </div>

      <div class="ico-triangle-down" @click="action('decrement')">
      </div>
    </div>

    <div v-if="ticker" class="input-ticker-actions">
      <div class="input-ticker__item" @click="action('increment')">
        +
      </div>

      <div class="input-ticker__item" @click="action('decrement')">
        -
      </div>
    </div>

    <i v-if="dbEdit && flags.edit" class="ico-save" @click="handleSave">
    </i>
  </div>
</template>
