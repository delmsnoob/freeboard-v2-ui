<!--
  ...
  <modal
    v-if="modal.test.$status"
    v-model="modal.test.$status"
    title="Modal"
...
export default {
  ...
  computed: {
    modalBodyStyles () {
      let height = null
      let type = this.maxHeight ? 'max-height' : 'height'
      ...
      if (this.device.height > 799) {
        if (this.device.width > 991) {
          height = bodyHeight + 'px'
        }
      }
  ...
  methods: {
    ...
    async getModalHeight () {
      ...
      if ((modalInnerHeight + 30) >= this.device.height) {
    ...
    async getModalBodyHeight () {
      ...
      if (this.device.height > 799 && this.device.width > 991) {
        if (this.$refs.modalFooter) {
          const modalFooter = this.$refs.modalFooter.offsetHeight
          this.modalBody = this.device.height - modalHeader - modalFooter - padding
        } else {
          this.modalBody = this.device.height - modalHeader - padding
        }
      }
    },
